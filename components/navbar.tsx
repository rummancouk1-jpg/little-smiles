    ...
    >
      <div className="mx-auto flex h-18 w-full max-w-7xl items-center justify-between gap-3 px-4 sm:h-20 sm:px-6 lg:gap-4 lg:px-8">
        <Link
          ...
          className="inline-flex items-center rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-walnut/30"
        >
          <Image
        ...
        <nav className="hidden items-center gap-8 xl:gap-10 lg:flex">
          {navLinks.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className="text-sm font-medium tracking-[0.03em] text-ink-base/76 transition-colors duration-200 hover:text-ink-walnut"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 lg:flex">
          ...
          <Button
            asChild
            className="h-11 rounded-full border-transparent bg-ink-walnut px-5 text-sm font-medium text-ink-foreground shadow-[0_12px_30px_-20px_rgba(47,38,36,0.5)] transition-[transform,box-shadow,background-color] duration-300 hover:-translate-y-0.5 hover:bg-ink-espresso hover:shadow-[0_18px_36px_-22px_rgba(47,38,36,0.6)]"
          >
            <Link
        ...
        <div className="flex items-center gap-2 lg:hidden">
          ...
          <Link
            ...
            className="inline-flex size-11 items-center justify-center rounded-full border border-transparent bg-ink-walnut text-ink-foreground shadow-[0_10px_24px_-16px_rgba(47,38,36,0.55)] transition-[transform,background-color,box-shadow] duration-200 hover:-translate-y-0.5 hover:bg-ink-espresso hover:shadow-[0_14px_28px_-18px_rgba(47,38,36,0.65)]"
          >
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent
              side="right"
              className="w-[86%] border-l-ink-base/12 bg-atmosphere-veil/96 px-5 pt-[max(1.25rem,env(safe-area-inset-top,0px))] pb-[max(1.25rem,env(safe-area-inset-bottom,0px))] backdrop-blur-xl sm:max-w-xs"
            >
              <SheetHeader className="px-0 pb-4">
                <SheetTitle className="text-ink-walnut">Little Smiles</SheetTitle>
              </SheetHeader>
              <nav className="flex flex-col gap-2">
                {navLinks.map((item) => (
                  <SheetClose asChild key={item.label}>
                    <Link
                      ...
                      className="min-h-12 rounded-xl px-3.5 py-3 text-[1.02rem] font-medium text-ink-base/82 transition-colors hover:bg-surface-hover hover:text-ink-walnut"
                    >
                      {item.label}
                    </Link>
                  </SheetClose>
                ...
                <SheetClose asChild>
                  <Link
                    href="/cart"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex min-h-12 items-center justify-between gap-3 rounded-xl px-3.5 py-3 text-[1.02rem] font-medium text-ink-base/82 transition-colors hover:bg-surface-hover hover:text-ink-walnut"
                  >
                    <span className="inline-flex items-center gap-2.5">
                      <ShoppingBag
                        className="size-[1.05rem] shrink-0 text-ink-base/72"
                        aria-hidden
                      />
                      Cart
                    </span>
                    {isCartReady && totalQuantity > 0 ? (
                      <span className="rounded-full bg-ink-walnut px-2.5 py-0.5 text-xs font-semibold tabular-nums text-ink-foreground">
                        {totalQuantity > 99 ? "99+" : totalQuantity}
                      </span>
                    ) : null}
                  </Link>
              ...
              <div className="mt-5 space-y-3">
                <SheetClose asChild>
                  <Button
                    asChild
                    className="h-12 w-full rounded-full bg-ink-walnut px-5 text-[0.95rem] font-medium text-ink-foreground shadow-[0_14px_34px_-20px_rgba(47,38,36,0.56)] transition-[transform,box-shadow,background-color] duration-300 hover:-translate-y-0.5 hover:bg-ink-espresso hover:shadow-[0_18px_40px_-22px_rgba(47,38,36,0.66)]"
                  >
                    <Link
                      href="/cart"
                      onClick={() => setIsMobileMenuOpen(false)}
                    ...
                    >
                      ...
                      View cart
                      {isCartReady && totalQuantity > 0 ? (
                        <span className="rounded-full bg-ink-foreground/15 px-2 py-0.5 text-xs font-semibold tabular-nums">
                          {totalQuantity > 99 ? "99+" : totalQuantity}
                        </span>
                      ) : null}
                    </Link>
